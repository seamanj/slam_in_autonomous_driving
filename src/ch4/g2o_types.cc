...
void EdgeInertial::linearizeOplus() {
    ...
    const Mat3d invJr = SO3::leftJacobianInverse(er).transpose();  // 右雅可比逆 = 左雅可比逆的转置
    ...
    // dR/dbg1, 4.45
    _jacobianOplus[2].block<3, 3>(0, 0) = -invJr * eR.inverse().matrix() * SO3::leftJacobian((dR_dbg * dbg).eval()).transpose() * dR_dbg;
    // dv/dbg1
