...
int PacketsParser::Setup(const VelodyneConfig &conf) {
    ...
    for (auto &rings_pointcloud : organized_raw_pointcloud_) {
        rings_pointcloud = std::make_shared<FullPointCloudType>();
    }
...
// 修改 1: 更新函数签名以使用 ROS 2 类型
void PacketsParser::PaddingPointCloud(const velodyne_msgs::msg::VelodyneScan::SharedPtr &scan_msg, 
                                      FullCloudPtr &out_pc_msg_ptr) {
    ...
    // 使用 pcl_conversions 转换 ROS 2 header 到 PCL header
    pcl_conversions::toPCL(scan_msg->header, out_pc_msg_ptr->header);
...
// 修改 3: 更新 Unpack 函数签名
void PacketsParser::Unpack(const velodyne_msgs::msg::VelodynePacket &pkt, 
                           const builtin_interfaces::msg::Time &msg_stamp,
    ...
    // 修改 4: ROS 2 时间转换
    const double pkt_time = pkt.stamp.sec + pkt.stamp.nanosec * 1e-9;
    const double msg_stamp_time = msg_stamp.sec + msg_stamp.nanosec * 1e-9;
    ...
    for (int block = 0; block < kBlocksPerPacket; block++) {
        ...
        for (int firing = 0, k = 0; firing < kVLP16FiringsPerBlock; ++firing) {
            for (int dsr = 0; dsr < kVLP16ScanPerFiring; ++dsr, k += kRawScanSize) {
                ...
                // 修改 5: 使用 ROS 2 时间计算差值
                point.time = static_cast<float>(point_stamp_us - msg_stamp_time * 1e6);
...
void PacketsParser::ArrangePointcloud(const std::vector<FullCloudPtr> &rings_pointcloud, FullCloudPtr &out_pc_ptr) {
    ...
    for (size_t i = 0; i < rings_pointcloud.size(); ++i) {
        if (rings_pointcloud[i]->points.size() <= 0) {
            // 修改 6: 使用 LOG(ERROR) 替代 ROS_ERROR_STREAM
            LOG(ERROR) << "lidar points in ring " << i << " is empty";
        }
