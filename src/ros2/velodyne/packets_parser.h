...
#include <builtin_interfaces/msg/time.hpp>  // 新增
...
class PacketsParser {
   public:
    ...
    // 修改：使用 ROS 2 的消息类型
    void PaddingPointCloud(const velodyne_msgs::msg::VelodyneScan::SharedPtr &scan_msg, 
   ...
   private:
    ...
    void ArrangePointcloud(const std::vector<FullCloudPtr> &rings_pointcloud, 
                          FullCloudPtr &out_pc_ptr);
